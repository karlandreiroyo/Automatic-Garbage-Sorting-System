...
- **Local backend:** In `backend/.env` set `ARDUINO_PORT=` your port (e.g. COM5, COM8 — any COM port), `ARDUINO_BAUD=9600`, and **`ARDUINO_LOCAL=true`**. Then start backend with `npm start`; the backend will open the serial port and Bin Monitoring will show "Serial connected".
...
2. Set your Railway backend URL and COM port, then run the bridge:
   ...
   $env:ARDUINO_PORT="COM5"   # or COM8, etc. — use the port your Arduino is on
