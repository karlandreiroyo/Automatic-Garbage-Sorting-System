...
const SearchIcon = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"></circle><path d="m21 21-4.35-4.35"></path></svg>;
...
const CollectionHistory = () => {
  ...
  // Validation: Ensure filter value is valid
  ...
  // Filter and search logic with validation
  ...
  const stats = useMemo(() => {
    ...
    // Count this week (simple validation - last 7 days would need proper date parsing)
  ...
  const isValidPage = safePage === currentPage;
    ...
    <div className="history-page">
        ...
        <div className="stats-row">
          <div className="stats-box">
            ...
            <div className="stats-content">
              <span className="stats-label">Total<br/>Collections</span>
          ...
          <div className="stats-box">
            ...
            <div className="stats-content">
              <span className="stats-label">Total<br/>Weight</span>
              <span className="stats-value">{stats.totalWeight} <small>kg</small></span>
            </div>
          </div>
          <div className="stats-box">
            <div className="stats-icon-bg bg-purple-light"><CalendarIcon /></div>
            <div className="stats-content">
              <span className="stats-label">This<br/>Week</span>
      ...
      {!hasData ? (
        <div className="empty-state-container">
          <div className="empty-icon-large"><AlertIcon /></div>
          <h3>No Collection History</h3>
          <p>There are no collection records available at the moment.</p>
        </div>
      ) : !hasFilteredResults ? (
        <div className="empty-state-container">
          <div className="empty-icon-large"><SearchIcon /></div>
              ...
              <div key={item.id} className="history-list-card">
                ...
                <div className="card-middle">
                  ...
                  <div className="card-meta-row">
                    <div className="meta-item"><CalendarIcon /> <span>{item.date || 'N/A'}</span></div>
                    <div className="meta-item"><ClockIcon /> <span>{item.time || 'N/A'}</span></div>
                  </div>
                  <div className="card-meta-row">
                    <div className="meta-item"><UserIcon /> <span>Collected by: {item.collector || 'Unknown'}</span></div>
                    <div className="meta-item"><SmallTrashIcon /> <span>Weight: {item.weight || 'N/A'}</span></div>
                  </div>
                </div>
                <div className="card-right">
                  <span className="status-completed"><CheckCircleIcon /> {item.status || 'Unknown'}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Validation warning (dev only - remove in production) */}
      {!isValidPage && (
        <div className="validation-warning" style={{display: 'none'}}>
          Page validation: Requested page {currentPage} adjusted to {safePage}
        </div>
      )}
    </div>
  );
