...
wss.on("error", (err) => {
  if (err && err.code === "EADDRINUSE") {
    console.error(`WebSocket server already running on ws://localhost:${PORT}`);
    process.exit(0);
  }
...
wss.on("connection", (ws) => {
  ...
  ws.on("error", (err) => {
    // Keep server alive even if a client disconnects/errors unexpectedly.
  ...
  ws.on("message", (raw) => {
    try {
      ...
      // Optional reset from frontend after drain.
      ...
      // Ignore unsupported typed messages; detection payloads from Python are untyped.
